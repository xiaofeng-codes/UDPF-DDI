...
def subtreeExtractor(data_type, uni_mol_feat, id_list, drug_id, edge_index, rel_index, shortest_paths, num_rel,
    ...
    for d in tqdm(drug_id, desc=f"Generating {data_type} sub_graph", ncols=150):
        ...
        node_id = [id_list[id] for id in node_idx]

        subgraphs[
            d] = node_feat, new_s_edge_index, new_s_rel, mapping_list, s_edge_index, s_value, s_rel, max_degree, node_id
...
class DrugDataset(Dataset):
    def __init__(self, drug_list, all_molfeature, uni_mol_dict, uni_atom_dict, ddi_subgraph, tri_list, ratio=1.0,
        ...
        self.log_file = 'batch_drugs.csv'
        header = ['Drug1_ID', 'Drug2_ID', 'Y']
        pd.DataFrame(columns=header).to_csv(self.log_file, index=False)
    ...
    def collate_fn(self, batch):
        ...
        df = pd.DataFrame(batch, columns=['Drug1_ID', 'Drug2_ID', 'Y'])

        df[['Drug1_ID', 'Drug2_ID', 'Y']].to_csv(self.log_file, mode='a', header=False, index=False)
    ...
    def get_bipartite_graph(self, d1, d2):
        x1 = np.arange(0, len(self.all_molfeature[d1][1]))

        x2 = np.arange(len(self.all_molfeature[d1][1]),
                       len(self.all_molfeature[d1][1]) + len(self.all_molfeature[d2][1]))
        edge_list = torch.LongTensor(np.meshgrid(x1, x2))
        edge_list = torch.stack([edge_list[0].reshape(-1), edge_list[1].reshape(-1)])

        batch_id = torch.cat([
            torch.full((len(self.all_molfeature[d1][1]),), 1, dtype=torch.long),
            torch.full((len(self.all_molfeature[d2][1]),), 2, dtype=torch.long)
        ])

        bi_features = torch.cat((self.all_molfeature[d1][1], self.all_molfeature[d2][1]), dim=0)

        return Data(x=bi_features, edge_index=edge_list, batch_id=batch_id)
...
def data_unzip(data, dataset_name):
    data_tup = [(h, t, int(r), n) for h, t, r, n in
                zip(data['Drug1_ID'], data['Drug2_ID'], data['Y'], data['Neg samples'])]

    data_set = []
    only_rel_set = []
    if dataset_name == 'deng':
        for h, t, r, n in data_tup:
            data_set.append((h, t, r))
            only_rel_set.append((h, t, r))

        only_rel_set = data_set
        ...
        for h, t, r, n in data_tup:
            ...
            if Ntype == 'h':
                data_set.append((Neg_ID, t, r))
                only_rel_set.append((Neg_ID, t, 65))

            else:
                data_set.append((h, Neg_ID, r))
                only_rel_set.append((h, Neg_ID, 65))

    return data_set, only_rel_set
...
def load_data(args, batch_size, fold_i):
    ...
    df_ddi_val = pd.read_csv(f'dataset/{dataset}/val_fold{fold_i}.csv', dtype=str)
    df_ddi_test = pd.read_csv(f'dataset/{dataset}/test_fold{fold_i}.csv', dtype=str)

    train_tup, train_only_pos = data_unzip(df_ddi_train, dataset)
    val_tup, val_only_pos = data_unzip(df_ddi_val, dataset)

    test_tup, test_only_pos = data_unzip(df_ddi_test, dataset)

    train_subgraphs, train_max_degree, train_max_rel_num = generate_k_subgraphs(df_drugs_smiles, uni_mol_dict,
                                                                                train_only_pos, args, 'train')
